...
ident.cellIdentity <- function(object, 
  ...
  for(i in 1:length(batch_list)) {
    ...
    batch_list[[i]] <- Seurat::FindVariableFeatures(batch_list[[i]], nfeatures = nfeatures, verbose = FALSE, clip.max = sqrt(NN))
    batch_list[[i]] <- batch_list[[i]] %>%
  ...
  if (!is.null(vargenes) == TRUE) {
    ...
    vargenes <- Reduce(union, lapply(batch_list, function(x) x@assays[["RNA"]]@var.features))
  } else if (select.var == "select") {
    vargenes <- Seurat::SelectIntegrationFeatures(batch_list, nfeatures = nfeatures)
  }

  vargenes <- rownames(object@norm.data[[1]])[which(rownames(batch_list[[1]]@assays$RNA@counts) %in% vargenes)]
  var_list <- lapply(var_list,
                     function(x) rownames(object@norm.data[[1]])[which(rownames(batch_list[[1]]@assays$RNA@counts) %in% x)])
...
run.Integrate <- function(object, 
  ...
  result_acpca <- RSpectra::eigs_sym(as.matrix(d), k = npcs, which = "LA") %>% '[['('vectors') %>% as.matrix()
  L = as.matrix(Matrix::crossprod(result_acpca, do.call(cbind, lapply(object@norm.data, function(x) x[object@vargenes, ]))))

  t2 = Sys.time()
  print(t2-t1)
  object@variation <- as.matrix(d)
  object@projection <- t(result_acpca)
...
compute.Similarity <- function(object, 
  ...
  for (i in 1:N) {
    ...
    vargs <- object@var_list[[i]]
    norms <- lapply(object@norm.data,
                    function(x) cosineNorm(x[vargs, ]))
...
#' @param T_th The similarity threshold for filtering cells in the cPCA space.
#'
